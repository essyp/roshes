...
<div id="content" class="ui-content ui-content-aside-overlay">
                ...
                <div class="ui-content-body">
                    ...
                    <div class="ui-container">
                        <div class="row">
                            <form id="status_form" action='{{url("admin/gallery-status")}}' method="POST">
                                {{ csrf_field() }}
                            <div class="col-sm-12">
                                <div class="mbot-20">
                                    <div class="btn-group">
                                    ...
                                    <button type="button" data-toggle="modal" data-target="#new" class="btn btn-default">Upload to Gallery</button>
                                </div>
                                <section class="panel">
                                    ...
                                    <div class="panel-body">
                                        <table id="datatable" class="table responsive-data-table table-striped">
                                            ...
                                            <tr>
                                                ...
                                                <td><img src="{{asset('images/gallery/'.$bl->image)}}" style="max-height: 60px; max-width: 60px"></td>
                                                <td>{{$bl->project->title}}</td>
                                                @if($bl->status==ACTIVE)
        ...
        <div class="modal-content">
            ...
            <div class="form">
                        ...
                        <div class="form-group">
                            <label for="project_id" class="control-label col-lg-3">Project</label>
                            <div class="col-lg-9">
                       ...
                       <div class="form-group">
                            <label for="image" class="control-label col-lg-3">Select Images</label>
                            <div class="col-lg-6">
                                <input class=" form-control" id="imgInp" accept="image/*" name="images[]" type="file" multiple required="required"/>
                            </div>
                        </div>
...
<div  id="editmodal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Update Gallery Image</h4>
            </div>
            ...
            <div class="form">
                        ...
                        <div class="form-group">
                            <label for="project_id" class="control-label col-lg-3">Project</label>
                            <div class="col-lg-9">
                                <select class=" form-control" id="editproject_id" name="project_id">
                                @foreach($projects as $ch)
                                 <option value="{{$ch->id}}">{{$ch->title}}</option>
                                 @endforeach
                                </select>
...
$('#new-form').submit(function(e){
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.status == "success"){
					...
					$( "#datatable" ).load( "{{url('/admin/gallery')}} #datatable" );
                    close_loader('#page');
                    } else{
    ...
    function update(event){
        ...
        $('#editproject_id').val(event.project_id)
        $('#editid').val(event.id)
        $("#editimage").attr('src', "{{asset('images/gallery')}}/"+event.image)
